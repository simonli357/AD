...
void Track::read_graph() {
    ...
    for (tinyxml2::XMLElement* nodeElem = graphElem->FirstChildElement("node");
         nodeElem;
         nodeElem = nodeElem->NextSiblingElement("node"))
    {
        ...
        for (tinyxml2::XMLElement* dataElem = nodeElem->FirstChildElement("data");
             dataElem;
    ...
    for (tinyxml2::XMLElement* edgeElem = graphElem->FirstChildElement("edge");
         edgeElem;
         edgeElem = edgeElem->NextSiblingElement("edge"))
    {
        ...
        if (sIt == idToVertex.end() || tIt == idToVertex.end()) {
            std::cerr << "Edge references unknown node: " 
