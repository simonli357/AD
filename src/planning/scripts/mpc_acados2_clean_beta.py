...
class Optimizer(object):
    ...
    def create_solver(self, config_path='config/mpc_config25.yaml'):
        ...
        L_f = 0.15    # distance from center-of-mass to front axle (in meters)
        L_r = 0.108   # distance from center-of-mass to rear axle (in meters)
        self.L = L_f + L_r  # total wheelbase
        ...
        Q = np.array([[self.x_cost, 0.0, 0.0],
                      [0.0, self.y_cost, 0.0],
                      [0.0, 0.0, self.yaw_cost]])*1
        R = np.array([[self.v_cost, 0.0],
                      [0.0, self.steer_cost]])*1
        ...
        ### N
        ocp.cost.yref_e = x_ref # yref_e is the reference for the last stage
    ...
    def update_and_solve(self):
        ...
        for j in range(self.N):
            if self.target_waypoint_index+j >= self.state_refs.shape[0]:
                ...
                self.solver.set(j, 'yref', np.concatenate((self.next_trajectories[j], self.next_controls[j])))
