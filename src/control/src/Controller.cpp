...
public:
    ...
    void check_car() {
        ...
        for (int i = start_idx; i < static_cast<int>(start_idx + 0.5 * PathManager::density); i++) {
            ...
            if (PathManager::attribute_cmp(i, PathManager::ATTRIBUTE::HIGHWAYLEFT)) { // if on right side of highway, overtake on left
                density *= 1/1.33;
            ...
            else if (PathManager::attribute_cmp(i, PathManager::ATTRIBUTE::HIGHWAYRIGHT)) { // if on left side of highway, overtake on right
                right = true; 
                on_highway = true;
                density *= 1/1.33;
                // PathManager::overtake_end_index_scaler *= 1.5;
