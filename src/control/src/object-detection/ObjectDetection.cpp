...
void ObjectDetection::detect_traffic_lights() {
	...
	if (world.stopsign_flag != OBJECT::NONE && world.stopsign_flag != OBJECT::LIGHTS && world.stopsign_flag != OBJECT::REDLIGHT && world.stopsign_flag != OBJECT::GREENLIGHT && world.stopsign_flag != OBJECT::YELLOWLIGHT) {
	...
	if (sign_index >= 0)
		is_red = true; // consider LIGHTS as red light
	if (sign_index < 0)
		sign_index = utils.object_index(OBJECT::GREENLIGHT);
	if (sign_index < 0)
		sign_index = utils.object_index(OBJECT::YELLOWLIGHT);
	if (sign_index >= 0 && !relocalized && world.stopsign_flag == OBJECT::NONE) {
		...
		if (dist < MAX_SIGN_DIST && dist > MIN_SIGN_DIST) {
			...
			if (sign_in_path(sign_index, dist + 0.2)) {
				...
				if (world.sign_relocalize) {
					...
					relocalized = world.sign_based_relocalization(light_pose, intersection_signs, sign_type);
				}
			}
		}
	}
...
void ObjectDetection::detect_cars() {
	...
	if (car_index >= 0) { // if car detected
		...
		utils.debug(
			"CHECK_CAR(): detected car at a distance of: " + std::to_string(dist) + ", closest index: " + std::to_string(closest_idx) + ", end index: " + std::to_string(path_manager.overtake_end_index), 4);
		...
		if (dist < MAX_CAR_DIST && dist > 0 && closest_idx >= path_manager.overtake_end_index + safety_dist * path_manager.density) { // if car is within range and ahead of ego car
			...
			for (int i = closest_idx; i < look_ahead_index; i++) { // iterate over waypoints in front of car, compute distance to car
				...
				Eigen::Vector2d adj_point = (path_manager.state_refs.block(i, 0, 1, 2).transpose().eval() + (path_manager.normals.block(i, 0, 1, 2).transpose().eval() * LANE_OFFSET * sign));
			...
			// utils.debug("CHECK_CAR(): closest waypoint to detected car: " + std::to_string(min_index) + ", at " + std::to_string(path_manager.state_refs(min_index, 0)) + ", " +
			// std::to_string(path_manager.state_refs(min_index, 1)), 3); utils.debug("CHECK_CAR(): min dist between car and closest waypoint: " + std::to_string(min_dist) + ", same lane: " +
			// std::to_string(detected_car_state == DETECTED_CAR_STATE::SAME_LANE), 3);
			if (detected_car_state == DETECTED_CAR_STATE::SAME_LANE) {
				if (idx < path_manager.state_refs.rows() && !path_manager.attribute_cmp(idx, path_manager.ATTRIBUTE::DOTTED) && !path_manager.attribute_cmp(idx, path_manager.ATTRIBUTE::DOTTED_CROSSWALK) &&
					!path_manager.attribute_cmp(idx, path_manager.ATTRIBUTE::HIGHWAYLEFT) && !path_manager.attribute_cmp(idx, path_manager.ATTRIBUTE::HIGHWAYRIGHT)) {
				...
				if (dist < MAX_TAILING_DIST) {
					utils.debug("CHECK_CAR(): NOT_SURE: dist = " + std::to_string(dist) + ", stopping... min_dist: " + std::to_string(min_dist) + ", min_dist_adj: " + std::to_string(min_dist_adj) +
									", car pose: (" + std::to_string(car_pose[0]) + ", " + std::to_string(car_pose[1]) + ")",
								2);
					current_state[CAR_DETECTED_ON_SAME_LANE] = true;
					current_state[LANE_IS_DOTTED] = false;
					return;
