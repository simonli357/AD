...
Utility::Utility(ros::NodeHandle &nh_, bool pubOdom) : nh(nh_), pubOdom(pubOdom), object_detection_time(ros::Time::now()), x0(0.0), y0(0.0) {
...
void Utility::fetch_run_params() {
	...
	auto dist = [](const Point &a, const Point &b) { return std::hypot(a.x - b.x, a.y - b.y); };

	auto is_in_circle = [&](const Circle &c, const Point &p) { return dist(c.center, p) <= c.radius + 1e-9; };

	auto make_circle_two = [&dist](const Point &a, const Point &b) {
		Point center{(a.x + b.x) / 2, (a.y + b.y) / 2};
		double radius = dist(center, a);
		return Circle{center, radius};
	};

	auto make_circle_three = [&dist](const Point &A, const Point &B, const Point &C) {
		double d = 2 * (A.x * (B.y - C.y) + B.x * (C.y - A.y) + C.x * (A.y - B.y));
		if (std::abs(d) < 1e-6)
			return Circle{{0, 0}, 0}; // degenerate
	...
	std::function<Circle(std::vector<Point>::iterator, std::vector<Point>::iterator, std::vector<Point>)> mec;
	mec = [&](auto begin, auto end, std::vector<Point> R) -> Circle {
		if (begin == end || R.size() == 3) {
			if (R.empty())
				return {{0, 0}, 0};
			if (R.size() == 1)
				return {R[0], 0};
			if (R.size() == 2)
				return make_circle_two(R[0], R[1]);
			return make_circle_three(R[0], R[1], R[2]);
		}

		Point p = *--end;
		Circle c = mec(begin, end, R);
		if (is_in_circle(c, p))
			return c;

		R.push_back(p);
		return mec(begin, end, R);
	};

	Circle result = mec(points.begin(), points.end(), {});

	std::cout << "Utility::fetch_run_params: number of pts: " << samples.size() << std::endl;
	std::cout << "Utility::fetch_run_params: MEC center: (" << result.center.x << ", " << result.center.y << "), radius: " << result.radius << std::endl;
	std::cout << "Utility::fetch_run_params: now waiting for IMU" << std::endl;
	std::cout << "Utility::fetch_run_params: IMU initialized, now calculating yaw" << std::endl;

	if (std::isnan(result.center.x) || std::isnan(result.center.y)) {
		debug("Utility:: invalid gps data", 1);
		return;
	}

	this->x0 = result.center.x;
	this->y0 = result.center.y;
...
void Utility::odom_pub_timer_callback(const ros::TimerEvent &) { publish_odom(); }

void Utility::sign_callback(const utils::Sign::ConstPtr &msg) { process_sign_data(*msg); }
void Utility::process_sign_data(const utils::Sign &msg) {
	...
	for (int i = 0; i < num_obj; i++) {
		...
		// std::cout << "sign_callback(): detected object: " << OBJECT_NAMES[type] << ", confidence: " << confidence << ", latency: " << latency << ", type: " << type << ", iscar: " << is_car << ",
		// worldstates: (" << world_states[0] << ", " << world_states[1] << ")" << std::endl;
			...
			if (is_known_static) {
				...
				if (helper::get_min_object_index(sign_pose, relevant_signs, min_index, min_error_sq, 0.357)) {
					...
					if (yaw_error < 45 * M_PI / 180) {
						Tracking::create_known_static_object(static_cast<OBJECT>(type), world_states[0], world_states[1], Sensing::yaw, confidence, relevant_signs[min_index]);
...
void Utility::lane_callback(const utils::Lane2::ConstPtr &msg) { process_lane_data(*msg); }
void Utility::lane_center_offset_callback(const std_msgs::Float32::ConstPtr &msg) { lane_center_offset = msg->data; }
...
void Utility::reset_odom() { set_initial_pose(0, 0, 0); }
int Utility::update_states_rk4(double speed, double steering_angle, double dt) {
	...
	double beta = 0;
	if (Tunable::use_beta)
		beta = atan((l_r / WHEELBASE) * tan(delta_rad));
...
void Utility::publish_cmd_vel(double steering_angle, double velocity, bool clip) {
	if (velocity < -3.5)
		velocity = maxspeed;
	if (clip) {
		if (steering_angle > HARD_MAX_STEERING)
			steering_angle = HARD_MAX_STEERING;
		if (steering_angle < -HARD_MAX_STEERING)
			steering_angle = -HARD_MAX_STEERING;
	}
...
std::array<double, 3> Utility::get_real_states() const { return {gps_x, gps_y, Sensing::yaw}; }
void Utility::spin() {
