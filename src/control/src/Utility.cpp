...
void Utility::process_lane_data(const utils::Lane3& msg) {
    {
        ...
        if (msg.good_left && msg.good_right && msg.straight_lane && std::abs(msg.straight_lane_angle) < Tunable::lane_yaw_reset_thresh1 * M_PI / 180.0) {
            const double current_straight_lane_angle = msg.straight_lane_angle;
            const double avg_straight_lane_angle =
                good_yaw_count > 0 ? (last_straight_lane_angle + current_straight_lane_angle) / 2.0
                                   : current_straight_lane_angle;
            last_straight_lane_angle = current_straight_lane_angle;
            good_yaw_count++;
            if (good_yaw_count < 2) return; // need 2 consecutive messages with straight lane angle to reset yaw
            const double current_yaw = Sensing::yaw.load(std::memory_order_relaxed);
            double nearest_direction_yaw = helper::nearest_direction(current_yaw);
            double lane_based_yaw = nearest_direction_yaw - avg_straight_lane_angle;
            double yaw_error = helper::compare_yaw(lane_based_yaw, current_yaw);
            ...
            if (Sensing::apply_yaw_measurement(lane_based_yaw, Tunable::lane_yaw_reset_thresh2 * M_PI / 180.0, 1.0)) {
                ...
                debug("LANE_YAW_RESET(): SUCCESS: Resetting yaw to lane-based yaw: " + helper::d2str(lane_based_yaw*180/M_PI) + ", current yaw: " + helper::d2str(current_yaw*180/M_PI) + ", yaw_error: " + helper::d2str(yaw_error*180/M_PI) + ", straight_lane_angle: " + helper::d2str(avg_straight_lane_angle*180/M_PI), 1);
                good_yaw_count = 0;
                // emergency=true;
