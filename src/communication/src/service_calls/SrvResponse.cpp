#include "service_calls/SrvResponse.hpp"
...
SrvResponse::SrvResponse(Float32MultiArray &state_refs, Float32MultiArray &input_refs, Float32MultiArray &wp_attributes, Float32MultiArray &wp_normals)
	: state_refs(state_refs), input_refs(input_refs), wp_attributes(wp_attributes), wp_normals(wp_normals) {}

uint32_t SrvResponse::compute_lengths_length() {
    return lengths_length;
}

uint32_t SrvResponse::compute_data_length() {
    return data_length;
}

std::vector<uint8_t> SrvResponse::get_lengths() {
...
std::vector<uint8_t> SrvResponse::get_data() {
