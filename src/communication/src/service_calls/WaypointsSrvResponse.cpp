...
WaypointsSrvResponse::WaypointsSrvResponse(uint8_t &data_type, Float32MultiArray &state_refs, Float32MultiArray &input_refs, Float32MultiArray &wp_attributes,
										   Float32MultiArray &wp_normals)
	: Encoder(data_type), state_refs(state_refs), input_refs(input_refs), wp_attributes(wp_attributes), wp_normals(wp_normals) {}

uint32_t WaypointsSrvResponse::compute_lengths_length() { return lengths_length; }

uint32_t WaypointsSrvResponse::compute_data_length() { return data_length; }

std::vector<uint8_t> WaypointsSrvResponse::get_lengths() {
...
std::vector<uint8_t> WaypointsSrvResponse::get_data() {
