...
ObjectMsg::ObjectMsg(const std::string &type, float x, float y) : type(type), x(x), y(y) {
...
std::vector<uint8_t> ObjectMsg::get_lengths() {
	...
	std::memcpy(lengths.data() + bytes_length * 2, &type_length, bytes_length);
	std::memcpy(lengths.data() + bytes_length * 3, &x_length, bytes_length);
	std::memcpy(lengths.data() + bytes_length * 4, &y_length, bytes_length);
	return lengths;
}
...
std::vector<uint8_t> ObjectMsg::get_data() {
	...
	size_t offset = 0;
	std::memcpy(data.data() + offset, type.data(), type_length);
    offset += type_length;
