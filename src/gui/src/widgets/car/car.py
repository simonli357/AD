    ...
    def draw_detected_objects(self, detected_data, road_msg_dict, object_dict):
        ...
        for i in range(len(detected_data)):
            ...
            if self.is_near(self.x_pos, self.y_pos, x_real, MapData.REAL_WORLD_HEIGHT.value - y_real, 3.0, 0.2):
                self.shader_renderer.text_renderer.render_text3D(f"{obj_name}: {confidence:.2f}", x, y, 9.0, self.width(), self.height(), RoadObjectsColor[label].value, self.proj_mat, self.view_mat)
